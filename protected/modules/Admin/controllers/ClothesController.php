...
class ClothesController extends Controller
{
	...
	* If creation is successful, the browser will be redirected to the 'view' page.
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['Clothes'];
			//var_dump($_FILES);die;
			//判断是否上传，有上传则处理文件上传项目图标 调用父类中的上传文件方法
			($_FILES['Clothes']['name']['picture'] != '') ? $this->UploadName($model, 'picture') : $i =1;
	...
	* @param integer $id the ID of the model to be deleted
	...
	{
		$model=new Clothes('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Clothes']))
			$model->attributes=$_GET['Clothes'];
	...
	* 不传参数默认为 images目录
	...
	public function moveFile($dir='')
	{
		if(is_object($this->file) && get_class($this->file)==='CUploadedFile'){
			if($dir=='') 
				$dir = Yii::app()->basePath."/../images/clothes/".$this->fileName;
			else 
				$dir .= $this->fileName;
	...
	* $dir 文件所在路径名
	...
	{
		if($dir == '')
			$dir = Yii::app()->basePath."/../images/clothes/".$file;
		else 
			$dir = $dir.$file;
